...
- **`synced_at` 은 신선도가 아니다.** 변경분만 upsert 하는 설계라 "마지막으로 값이 바뀐 시각"이다. 국회가 조용한 주에는 정상 동기화 중에도 몇 주 전으로 보이므로 기준일로 노출하면 안 된다. 신선도는 `bills_sync_runs`(실행 단위 메타)의 마지막 **성공** 시각으로만 판단하며, 로직은 `src/services/freshness.ts` 에 있다.
...
신선도 경고는 48시간 임계다. 크론이 일 1회라 1회 실패는 흡수해야 한다 — 상류(국회 API)가 출렁이는 날 같은 작업이 5초에서 74초까지 튀는 것이 관측됐다. 더 조이면 오탐이 나고, **오탐이 나는 경고는 곧 무시된다.** 상태를 표시하지 않는 도구(`bills_statistics`·`bills_daily_report`)에는 이 경고를 붙이지 않는다(`scopedStatusResult` vs `scopedResult`).
